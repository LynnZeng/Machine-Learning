...
Xtrn = pd.read_csv("X-trn-400.csv",header=None)
Ytrn = pd.read_csv("Y-trn-400.csv",header=None)
Xtst = pd.read_csv("X-tst-400.csv",header=None)
Ytst = pd.read_csv("Y-tst-400.csv",header=None)
...
plt.title('(400.csv-Entropy)Blue:traning set,Red:test set')
plt.show()

plt.plot(x,TrnAc_Gini,'b',x,TstAc_Gini,'r')
plt.ylabel('accuracy')
plt.xlabel('max tree depth')
plt.title('(400.csv-Gini)Blue:traning set,Red:test set')
plt.show()
